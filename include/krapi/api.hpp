...
#ifndef __krapi__api_hpp
#define __krapi__api_hpp
...
namespace krapi
{
    namespace rest
    {
        ...
        struct api
        {
            ...
            api(
                boost::asio::io_context &ioctx, std::string host, std::string port, std::string pk, std::string sk, std::size_t timeout, std::string client_api_string = "krapi-0.0.1");
            virtual ~api();
            ...
            result<add_order_t>
            add_order(
                const std::string &pair, const std::string &type // "buy" / "sell"
                ,
                const std::string &ordertype // "market" / "limit" / "stop-loss" / ...
                ,
                const std::string &volume, const std::string &price = std::string{} // required for limit/stop orders
                ,
                bool validate = false, add_order_cb cb = {});
